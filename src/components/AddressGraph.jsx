...
export default function AddressGraph() {
  ...
  const isInDateRange = useCallback(
    (headstone) =>
      !timeFilterEnabled ||
      (Number(headstone.BirthYear) >= selectedRange.min &&
        Number(headstone.DeathYear) <= selectedRange.max),
  ...
  useEffect(() => {
    ...
    svg
      .selectAll('.headstone-to-address')
      .data(graphData.plottedHeadstones)
      .enter()
      .append('path')
    ...
    svg
      .selectAll('.address-to-headstone')
      .data(graphData.plottedHeadstones)
      .enter()
      .append('path')
    ...
    const zoom = d3
      ...
      .on('zoom', (event) => {
        ...
        svg
          .selectAll('.headstone-to-address')
          .attr('d', (headstone) => generatePathData(headstone, headstone.AddressObj, event.transform));

        svg
          .selectAll('.address-to-headstone')
          .attr('d', (headstone) => generatePathData(headstone.AddressObj, headstone, event.transform));
  ...
  useEffect(() => {
    ...
    if (showAll) {
      svg
        .selectAll('.address-to-headstone,.headstone-to-address')
